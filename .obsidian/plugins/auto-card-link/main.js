...
var ObsidianAutoCardLinkSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("增强默认粘贴").setDesc("使用默认粘贴命令在编辑器中粘贴 url 时获取链接元数据").addToggle((val) => {
    ...
    new import_obsidian.Setting(containerEl).setName("在菜单项中添加命令").setDesc("是否在右键菜单中添加命令").addToggle((val) => {
      if (!this.plugin.settings)
        return;
...
var CodeBlockGenerator = class {
  ...
  convertUrlToCodeBlock(url) {
    return __async(this, null, function* () {
      ...
      if (!linkMetadata) {
        new import_obsidian2.Notice("无法获取链接数据");
    ...
    return __async(this, null, function* () {
      try {
        ...
        if (error instanceof NoRequiredParamsError) {
          ...
          console.log("代码块：卡片链接未知错误", error);
        }
      }
    });
  }
  ...
  constructor() {
    ...
    this.onEditorMenu = (menu) => {
      ...
      if (!((_a = this.settings) == null ? void 0 : _a.showInMenuItem))
        return;
      menu.addItem((item) => {
        item.setTitle("粘贴 URL 为卡片链接").setIcon("paste").onClick(() => __async(this, null, function* () {
      ...
      if (!navigator.onLine)
        return;
      menu.addItem((item) => {
        item.setTitle("增强所选 URL 为卡片链接").setIcon("link").onClick(() => {
          const editor = this.getEditor();
          if (!editor)
            return;
